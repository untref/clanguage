...
/***
 * This function makes a clone of the arguments (a new instance)
 *
 ...
 **/
ComplaxData* createCopy(ComplaxData* cmp ) {
  ComplaxData* result = (ComplaxData*)malloc( sizeof(ComplaxData) );
  result->anInteger = cmp->anInteger;
  result->arraySize = cmp->arraySize;
  result->anArray = cmp->anArray;
  result->anArray = (char*)malloc( cmp->arraySize );
  strcpy( result->anArray, cmp->anArray);
